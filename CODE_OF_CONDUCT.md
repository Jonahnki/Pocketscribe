...
This is research software, so two further expectations apply to participation here:

* **Attribute correctly.** Contributions that wrap or build on another tool must cite it.
  Misattributing someone else's method is a community problem, not only a technical one.
* **Do not overstate what the software shows.** Pocketscribe produces computational
  triage of predicted structures. Framing its output as experimental evidence, or as
  having clinical relevance, misleads the people who rely on it.
...
the maintainers responsible for enforcement by opening a confidential issue, or by
contacting the maintainer directly through the contact details on their GitHub profile.

<!--
MAINTAINER: replace the line above with a dedicated contact address before
announcing this project publicly. A Code of Conduct that does not say exactly where to
send a report is not usable by the person who needs it.
-->
